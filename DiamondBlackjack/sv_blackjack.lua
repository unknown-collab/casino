...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent("blackjack:notify",source,"~r~錯誤，無法讓您坐下來")
...
AddEventHandler("Blackjack:setBlackjackBet",function(gameId,betAmount,chairId)
    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount) then
						...
						if xPlayer.getMoney() >= betAmount then
						...
						TriggerClientEvent("blackjack:notify",source,"~g~下注籌碼： " .. tostring(betAmount) .. " 元")
						TriggerEvent("ldc:tttt1",xPlayer.name, betAmount)
						xPlayer.removeMoney(betAmount)
						else
						TriggerClientEvent("blackjack:notify",source,"~r~籌碼不足")
						end
                    else 
                        TriggerClientEvent("blackjack:notify",source,"~r~籌碼不足")
                    end
        ...
        TriggerClientEvent("blackjack:notify",source,"~r~投注錯誤!")
    end
end)
                ...
                if blackjackGameData[gameId] ~= nil then
                    for k,v in pairs(blackjackGameData[gameId]) do
                        if v ~= nil then
                            ...
                            betAmount = v[1]
                            -- print("betAmount: " .. tosring(betAmount))
                            -- print("v: " .. tostring(v))
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent("blackjack:notify",source,"~r~-"..tostring(lostAmount).." 元")
													local xPlayer = ESX.GetPlayerFromId(source)
													TriggerEvent("ldc:lose21",xPlayer.name, lostAmount)
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chatMessage', -1, "鑽石賭場 | " .. GetPlayerName(source) .. " 輸了 " .. tostring(getMoneyStringFormatted(lostAmount)) .. " !")
                                                    end
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~g~+"..tostring(potentialWinAmount).." 元")
															local xPlayer = ESX.GetPlayerFromId(source)
															xPlayer.addMoney(potentialWinAmount)
															TriggerEvent("ldc:win21",xPlayer.name, potentialWinAmount)
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "鑽石賭場 | " .. GetPlayerName(source) .. " 贏得了 " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " !")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
															...
															local xPlayer = ESX.GetPlayerFromId(source)
															TriggerEvent("ldc:lose21",xPlayer.name, "平手")
                                                            TriggerClientEvent("blackjack:notify",source,"~b~平手 錢由莊家收走")
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~r~-"..tostring(potentialPushAmount).." 元")
															local xPlayer = ESX.GetPlayerFromId(source)
															TriggerEvent("ldc:lose21",xPlayer.name, potentialPushAmount)
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "鑽石賭場 | " .. GetPlayerName(source) .. " 輸了 " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " !")
                                                            end
                                                        end
                                                    end
                                                end
